...
const { default: CompactEncrypt } = require('jose/jwe/compact/encrypt'); // eslint-disable-line import/no-unresolved
const { default: CompactSign } = require('jose/jws/compact/sign'); // eslint-disable-line import/no-unresolved
const { default: compactDecrypt } = require('jose/jwe/compact/decrypt'); // eslint-disable-line import/no-unresolved
const { default: compactVerify } = require('jose/jws/compact/verify'); // eslint-disable-line import/no-unresolved
const { default: decodeProtectedHeader } = require('jose/util/decode_protected_header'); // eslint-disable-line import/no-unresolved
const { JWEDecryptionFailed, JWKSNoMatchingKey, JWSSignatureVerificationFailed } = require('jose/util/errors'); // eslint-disable-line import/no-unresolved
...
class JWT {
  static async sign(payload, key, alg, options = {}) {
    ...
    return new CompactSign(Buffer.from(JSON.stringify(payload)))
      ...
      .sign(key instanceof KeyObject ? key : key.keyObject);
  }
  ...
  static async encrypt(cleartext, key, {
    ...
    return new CompactEncrypt(Buffer.from(cleartext))
      .setProtectedHeader(header)
      .encrypt(key instanceof KeyObject ? key : key.keyObject);
  }
